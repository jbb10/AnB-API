	...
	concatMap 
        ...
        (case t of 
           ...
           UnOp op m -> 
               ...
               if intrudible op then [([m],emptyFM)] else []
           _ -> []))
...
analz theo constraints (allmsg,undeciphered) msgs =
 ...
 (analz0 theo True (constraints,emptyFM,msgs,undeciphered,allmsg)))
...
analz0 theo@(fecTheo,(decanaTheo,_)) bool (constraints@(cs,ineq),sub,n:new,test,azed)
 ...
 | otherwise =  
    ...
    if (notOp n) then analz0 theo True (constraints,sub,new,test,addToMT azed n) else
       ...
       if elem sub ps 
       then analz0 theo True (constraints,sub,newnew emptyFM,[],addToMT azed n)
       else
