    ...
    Just 
       (\ number -> 
         let (id,((ls0,aem0),rs0,fresh_stuff)) = (goal2rule ("execCheck",([Plain ("iknows",[PCompT "reach_" [PVar "1172",PConst $ "rule"++(show number)]])],[]))) 
             (aem,ifsub) = preproConds (aem0,emptyFM)
...
pprintRules (algtheo,table)  cd normalize abstract rename rulez@(((id,((ls0,aem0),rs0,fresh_stuff)),number):rules) =
    let (aem,ifsub) = preproConds (aem0,emptyFM)
        ls = ifsubnstate ifsub ls0
 	label = ("iknows",[PCompT "reach_" [PConst id,PConst $ "rule"++(show number)]])
    ...
    (filter 
     (\ (msg,sub,ineq') ->
         (disjoint fresh_stuff 
          (concatMap (\ (x,t) -> x:(msg_vars t)) (fmToList sub)) )
      )
...
rplfstate a v i = map (substitute_fact (unitFM v (UnOp opNonce (BinOp opNAPair (Atom atom_incomparable a) (Var i)))))
...
rplfstatei a v i = map (substitute_fact (unitFM v (UnOp opNonce (BinOp opNAPair (Atom atom_incomparable a) (Number i)))))
