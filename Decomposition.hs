...
metaTopdec theo op (BinOp op' t1 t2) =
  ...
  concat [ interpret cases [t1,t2] emptyFM | ((op'',[v1,v2]),cases) <- theo op, op'==op'']
metaTopdec _ _ _ = [] 
...
decana2 decanaTheo fecTheo m =
  let ruleloop
        ...
        (checksubstineqs fecTheo ineq 
         (unifySF fecTheo emptyFM [(m,term)]))
...
checksubstineqs fecTheo ineqs = 
 (filter (isSatisfiable . snd)) .
    (map (\ sub -> 
            if checksub2 (fmToList sub) then error ("Now what is this: "++(show_sub sub)++" inequality: "++(show ineqs))
...
removeRuleVars fecTheo (t,sub,ineq) =
  ...
  let list = filter (not . isRuleVar . fst) (fmToList sub) in
...
groundnormalizationCF algTheo@(fecTheo,(_,(_,theoryOf))) t@(BinOp op m1 m2) =
 case theoryOf op of
  ...
  Just theo ->
   case concatMap 
       (\ (lhs,rhs) -> 
           [ deground (substitute sub rhs) | sub <- match fecTheo emptyFM [(t,lhs)]])
       theo of
