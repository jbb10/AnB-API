  ...
  in
   case lhswterms of
	...
	(\ (wfact,rest,overridden) -> 
              case catMaybes [(unify_facts freeAlg wfact lhswterm emptyFM)] of
	       [] -> []
	       sub0 -> 
...
match_rest fecTheo (state,(lhs,neg,cond,rhs,rtype)) =
  let domain = nub
      ...
      subs = 
        concatMap (\ neg_fact -> catMaybes ((\x -> map (\y -> unify_facts fecTheo x y emptyFM) (facts state)) (neg_fact))) neg
        --- concatMap (\x -> catMaybes $ map (\y -> unify_facts fecTheo x y emptyFM) (facts state)) neg
  in if (containsnonIDsub domain (map fmToList subs)) then []
...
analz_knowl veryLazy theo@(fecTheo,_) por (state,(lhs,neg,cond,rhs,rtype)) = 
 let mterms = filter isMTerm rhs
  ...
  if mterms==[] then 
    ...
    let ana = if veryLazy then [(cs state,emptyFM,ik state)]
 ...
 (foldl
  (\ possibilities constraint ->
	...
	(analz algtheo constraints (getallIK currentIK) (mtToList nik')))
     possibilities)
  [((([],snd constraints),emptyFM),(emptyIK,emptyIK,False))]
  (fst constraints))
...
match_msg_fact_must fecTheo (state,(lhs,neg,cond,rhs,rtype)) =
 ...
 in
  ...
  [M i real off recv mterm ses] -> 
    ...
    let csl=gencheck fecTheo (constraints++[mkDFromState [mterm] (ik state)],checkineqs fecTheo (ineq++cond)) emptyFM
