...
productname = "Open-Source Fixedpoint Model-Checker version 2014\n"
...
main 
 = do args <- getArgs
      ...
      putStr $ if (outt anbonp)==Isa then "Backend: Open-Source Fixedpoint Model-Checker version 2014 \n" else ""
      ---
      (case fileout onp of
	    Nothing -> return ()
...
mainWithArgs (onp,anbonp) (brothers::Maybe ((MVar Result,MVar Result))) iterative =
   do file <- if isAnB onp then 
      ...
      tfile <-  case theofile onp of 
                   ...
                   Just f  -> readFile f
      (algTheo,tab) 
           <- if tfile == "" then return ((fecStd,(decanaStd,cancellationStd)),emptyFM)
...
gentest = gencheck fecStd ([ mitestconstraint],[]) emptyFM
eagerly = null gentest

gentest2 = gencheck0 fecStd [] mitestconstraint ([],[]) emptyFM
